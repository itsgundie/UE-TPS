...
void UTPSGameSetting::ApplyNextOption()
{
    ...
    SerCurrentValue(Options[NextIndex].Value);
}

void UTPSGameSetting::ApplyPreviousOption()
{
    const int32 CurrentIndex = GetCurrentIndex();
    if (CurrentIndex == INDEX_NONE) return;
    const int32 PrevIndex = CurrentIndex == 0 ? Options.Num() - 1 : CurrentIndex - 1;
    SerCurrentValue(Options[PrevIndex].Value);
}
...
void UTPSGameSetting::SerCurrentValue(int32 Value)
{
    if (!Setter)
    {
